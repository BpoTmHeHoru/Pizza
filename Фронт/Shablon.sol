...
contract name {
   ...
   enum Roles {
    ...
    constructor(string[3] memory Fio, string memory login, string memory pass) {
    ...
    function Registr(string[3] memory Fio, string memory login, string memory pass) public {
        require(RegisteredUser[msg.sender] == false); // [-] You are not gay
        ...
        RegisteredUser[msg.sender] = true; // [+] You are gay, welcom to the clab
    }
    ...
    function (address _toaddr) public IsAdmin { 
        Users[_toaddr].Role = RoleAddNewAdmins.admin;
        Totaladmin++;
    }
...
//<Функционал менеджера>
    ...
    function AddNewItem(string memory Name, string memory Description, string memory Ingridients, uint Price, uint ItemId) 
    public IsAdmin {
        Itemes.push(Item(Name, Description, Ingridients, Price, ItemId));
    }
